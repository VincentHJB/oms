			...
			this.createCorePlug = function(){
				var params = {
					...
					onSuccess: function(file, response) {
						...
						if("success" == response){
							$("#uploadInf").append("<p>成功上传文件：" + file.name + "</p>");
						}else{
							$("#uploadInf").append("<p>文件上传失败：" + file.name + "</p>");
						}
						// 根据配置参数确定隐不隐藏上传成功的文件
						if(para.finishDel){
							// 移除效果
			...
			this.addEvent = function(){
				...
				$(".upload_btn").bind("click", function(e){
					...
					if(ZYFILE.funReturnNeedFiles().length > 0){
						...
						var curr = parseInt(ZYFILE.funReturnNeedFiles().length);
						console.log(total + curr)
						if((total + curr) > 5 || parseInt(cur_total)>=5){
